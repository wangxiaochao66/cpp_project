...
if __name__ == '__main__':
    ...
    parser.add_argument('estimated_files', help='estimated trajectory file (format: "timestamp tx ty tz qx qy qz qw")')
    parser.add_argument('estimated_filess', help='estimated trajectory file (format: "timestamp tx ty tz qx qy qz qw")')
    ...
    traj_ests = read_trajectory(args.estimated_files)
    traj_estss = read_trajectory(args.estimated_filess)
    ...
    result = evaluate_trajectory(traj_gt,
                                 ...
                                 float(args.offset),
                                 float(args.scale))
    results = evaluate_trajectory(traj_gt,
                                 traj_ests,
                                 int(args.max_pairs),
                                 args.fixed_delta,
                                 float(args.delta),
                                 ...
                                 float(args.offset),
                                 float(args.scale))
    resultss = evaluate_trajectory(traj_gt,
                                 traj_estss,
                                 int(args.max_pairs),
                                 args.fixed_delta,
                                 float(args.delta),
    ...
    stampss = numpy.array(results)[:,0]
    stampsss = numpy.array(resultss)[:,0]
    trans_error = numpy.array(result)[:,4]
    trans_errors = numpy.array(results)[:,4]
    trans_errorss = numpy.array(resultss)[:,4]
    rot_error = numpy.array(result)[:,5]
    rot_errors = numpy.array(results)[:,5]
    rot_errorss = numpy.array(resultss)[:,5]
    if args.save:
        f = open(args.save,"w")
    ...
    if args.plot:    
        ...
        ax.plot(stamps - stamps[0],trans_error,'-',color="orange")
        ax.plot(stampss - stampss[0],trans_errors,'-',color="red")
        ax.plot(stampsss - stampsss[0],trans_errorss,'-',color="green")
